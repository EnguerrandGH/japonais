...
let vocularFirstGroup = [["abiru","Se baigner"],["ageru","donner"],["akeru","ouvrir"],["atsumeru","rassembler","collecter"],["awateru","paniquer","s’agiter"],["butsukeru","frapper contre qqch"],["dekakeru","sortir de chez soi","aller se balader"],["dekiru","être capable","réussir"],["deru","sortir"],["eru","obtenir"],["fueru","augmenter","accroitre"],["fureru","toucher","frôler"],["fuseru","baisser sa tête"],
                        ...
                        ["kariru" ,"emprunter", "louer" ],["kasaneru" ,"entasser", "répéter"],["katameru" ,"durcir", "endurcir" ],["katazukeru" ,"achever (une tache)" ,"ranger", "mettre de l’ordre" ],["kazoeru", "compter", "nombrer" ],["kieru" ,"disparaitre", "s’éteindre" ],["kikaeru" ,"se changer (vêtement)" ],["kikoeru" ,"être audible", "arriver à entendre" ],["kimeru" ,"decider" ],["kinjiru" ,"interdire", "prohiber" ],["kireru" ,"se couper" ,"expirer" ,"s’énerver" ],["kiru" ,"s’habiller" ],["kizutsukeru" , "blesser", "faire du mal"],
...
function nextVocularFirstGroup () {
    ...
    if (vocularFirstGroupCopy.length != 0) {
        ...
        vocularFirstGroupCopy.splice(randomNumber, randomNumber + 1);
    }
    else {
...
function sameVocularFirstGroup(userInput) {
    if (listOfWords.includes(userInput))
    {
        console.log("ok");
        return true;
    }
...
solutionButton.addEventListener("click", () => {
    solutions.innerHTML = `${listOfWords}`;
});



translateInput.addEventListener('input', () => {
    translateInput.setCustomValidity('');
    translateInput.checkValidity();
});

translateInput.addEventListener('invalid', () => {
  if(translateInput.value === '') {
    translateInput.setCustomValidity(`Veuillez saisir une traduction non vide !`);
  } else {
    translateInput.setCustomValidity(`La traduction ne peut contenir que des lettres en minuscules ou majuscules. Essayez à nouveau.`);
  }
});